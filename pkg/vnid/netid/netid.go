...
func (r *NetIDRange) Set(base, size uint32) error {
	if base < vnid.MinVNID {
		return fmt.Errorf("invalid netid base, must be greater than %d", vnid.MinVNID)
	}
	if size == 0 {
		return fmt.Errorf("invalid netid size, must be greater than zero")
	}
	if (base + size - 1) > vnid.MaxVNID {
		return fmt.Errorf("netid range exceeded max value %d", vnid.MaxVNID)
	}
