...
func (n *NodeIPTables) Setup() error {
	...
	n.ipt.AddReloadFunc(func() {
		if err := n.syncIPTableRules(); err != nil {
			glog.Errorf("Reloading k8s-ovs iptables failed: %v", err)
		}
	})
...
func (n *NodeIPTables) syncLoop() {
	...
	for {
		<-t.C
		glog.V(6).Infof("Periodic k8s-ovs iptables sync")
		err := n.syncIPTableRules()
		if err != nil {
			glog.Errorf("Syncing k8s-ovs iptables failed: %v", err)
		}
	}
}
...
func (n *NodeIPTables) syncIPTableRules() error {
	...
	glog.V(3).Infof("Syncing k8s-ovs iptables rules")
